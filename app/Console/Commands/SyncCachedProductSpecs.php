...
class SyncCachedProductSpecs extends Command implements SignalableCommandInterface
{
    protected $signature   = 'products:sync-specs {limit=1800}';
    protected $description = 'Safely sync DigiKey specs into cached_products table';
    ...
    private function isForceStop(): bool
    {
        $val = (int) DB::table('system_settings')
            ->where('setting_key', 'digikey_specs_force_stop')
            ->value('setting_value');
    ...
    /**
     * Sleep for $ms milliseconds in 100 ms ticks, checking for abort each tick.
    ...
    /**
     * Print the current status of every configured DigiKey key pair.
     */
    private function printKeyStatuses(DigiKeyService $service): void
    {
        $this->line('');
        $this->line('┌─────────────────────────────────────────────────────┐');
        $this->line('│  DIGIKEY KEY STATUS                                 │');
        $this->line('├───────┬──────────────┬────────┬──────────┬──────────┤');
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            $this->info('================================================');
            $this->info('DIGIKEY SAFE SPECS SYNC STARTED');
            $this->info('================================================');
            ...
            // Show initial key roster
            $this->printKeyStatuses($service);

            CachedProduct::where(function ($q) {
                $q->whereNull('specs')
            ...
            ->chunkById(5, function ($products) use (
                ...
                foreach ($products as $product) {
                    ...
                    DB::table('system_settings')->updateOrInsert(
                        ['setting_key' => 'digikey_specs_last_id'],
                        ['setting_value' => $product->id, 'updated_at' => now()]
                    );

                    DB::table('system_settings')->updateOrInsert(
                        ['setting_key' => 'digikey_sync_stats'],
            ...
            DB::table('system_settings')->updateOrInsert(
                ['setting_key' => 'digikey_sync_last_summary'],
                ['setting_value' => json_encode([
            ...
            Mail::raw(
                ...
                function ($message) {
                    ...
                    $message->subject('DigiKey Sync Summary');
                }
            );
        }
