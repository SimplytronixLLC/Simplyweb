...
public function index()
{
	...
	public function user_message(){
		  ...
		  return view('admin.user_message',$data);
	}
	 
     public function send_message(Request $request){
	   if($request->id){
		   foreach($request->id as $val ){
			  $user = DB::table('users')->where('id',$val)->first();
			  if($user){
				$name     = $user->name;
				$email     = $user->email;
				...
				$common = new Common;
				echo $common->send_mail_multiple($name,$email,$subject,$messages);
			  }      
			} 
	   } 
	...
	public function about(){
         ...
         return view('admin.others.about',$data);
	}
    ...
    public function update_about(Request $request) {
		$settings = DB::table('settings')->where('id','=',1)->get()->first();
		 if ($file = $request->file('photo')){
		    if (file_exists(public_path('uploads/'.$settings->about_image)))  {
				@unlink(public_path('uploads/'.$settings->about_image));
			 }   
			$photo_name = rand().$request->file('photo')->getClientOriginalName();
			$destinationPath = public_path('/uploads');
			$file->move($destinationPath, $photo_name);
			$data['about_image'] = $photo_name;
        }  
   ...
   public function update_address(Request $request) {
	...
	public function update_social_setting(Request $request) {
		  ...
		  $data['tiktok_status'] = $request->tiktok_status;
		  $affected = DB::table('settings')->where('id', 1)->update($data);
          return redirect()->back()->with('message', 'Record Updated Successfully');  
	}
