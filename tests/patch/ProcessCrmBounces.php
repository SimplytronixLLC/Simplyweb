...
/**
 * Polls a dedicated bounce mailbox (e.g. bounces@simplytronix.com) over IMAP
 * for DSN (delivery-status-notification) messages, matches them back to the
 * original send via the Message-ID token stamped by CrmTrackedMail, and
 * updates the contact's bounce state.
 *
 ...
 *   CRM_BOUNCE_IMAP_HOST=mail.simplytronix.com
 *   CRM_BOUNCE_IMAP_PORT=993
 *   CRM_BOUNCE_IMAP_USER=bounces@simplytronix.com
 *   CRM_BOUNCE_IMAP_PASS=...
 *   CRM_BOUNCE_HARD_THRESHOLD=2
 */
class ProcessCrmBounces extends Command
{
    ...
    public function handle(): int
    {
        ...
        foreach ($unreadIds as $msgId) {
            ...
            $isDsn = $this->looksLikeDsn($structure);

            if (!$isDsn) {
                continue; // leave non-DSN mail alone, don't mark as seen
            }
            ...
            if ($token) {
                $log = CrmEmailLog::where('message_token', $token)->first();
                if ($log && $log->contact_id) {
                    $this->registerBounce($log->contact_id, $isHardBounce, $threshold);
                    $matched++;
                } else {
                    $unmatched++;
                    Log::warning("crm:process-bounces — token {$token} not found in crm_email_log");
                }
            } else {
                $unmatched++;
            }
    ...
    private function looksLikeDsn($structure): bool
    {
        // multipart/report; report-type=delivery-status is the RFC 3464 signature
        if (!isset($structure->type) || $structure->type !== 1 /* MULTIPART */) {
            return false;
        }
    ...
    {
        // DSNs usually quote the original Message-ID inside the
        // "Original-Message-Id:" or "Message-ID:" line of the embedded
        // message/rfc822 part. Some mail servers strip this, so we also
        // fall back to scanning for our uuid@domain pattern anywhere in body.
    ...
    {
        // RFC 3464 "Action:" field: failed = hard, delayed = soft/transient
        if (preg_match('/Action:\s*(failed|delayed)/i', $rawBody, $m)) {
            return strtolower($m[1]) === 'failed';
        }
        // Fallback heuristic if Action: is missing
    ...
    private function registerBounce(int $contactId, bool $isHard, int $threshold): void
    {
        ...
        if (!$isHard) {
            // Soft bounce: log it but don't count toward the hard threshold.
